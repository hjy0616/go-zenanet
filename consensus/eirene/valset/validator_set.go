...
// TendermintValidatorCopy converts the validator set to a format compatible
// with Tendermint validators
func (vals *ValidatorSet) TendermintValidatorCopy() []*Validator {
...
// UpdateFromTendermint updates the validator set from Tendermint validators
// This is used when syncing the validator set from Tendermint consensus
func (vals *ValidatorSet) UpdateFromTendermint(tmValidators []*Validator) error {
	if vals == nil {
		return fmt.Errorf("validator set is nil")
	}

	// Convert Tendermint validators to a format compatible with our validator set
	changes := make([]*Validator, len(tmValidators))
	for i, val := range tmValidators {
		changes[i] = val.Copy()
	}

	// Update our validator set with the changes
	return vals.UpdateWithChangeSet(changes)
}

// SyncFromTendermint completely replaces the current validator set with
// the provided Tendermint validator set
func (vals *ValidatorSet) SyncFromTendermint(tmValidators []*Validator) error {
	if vals == nil {
		return fmt.Errorf("validator set is nil")
	}

	// Create a new validator set with Tendermint validators
	newVals := NewValidatorSet(tmValidators)

	// Copy the new validator set to the current one
	vals.Validators = newVals.Validators
	vals.Proposer = newVals.Proposer
	vals.totalVotingPower = newVals.totalVotingPower
...
// VerifyTendermintUpdate checks if the update from Tendermint validators
// would change more than 1/3 of the total voting power, which is not allowed
// in a single update for safety reasons
func (vals *ValidatorSet) VerifyTendermintUpdate(tmValidators []*Validator) error {
	if vals == nil || len(vals.Validators) == 0 {
		// First time update is always allowed
		return nil
	}

	// Calculate current total voting power
	currentPower := vals.TotalVotingPower()

	// Create a map of current validators by address
	currentVals := make(map[common.Address]*Validator)
	for _, val := range vals.Validators {
		currentVals[val.Address] = val
	}

	// Calculate how much voting power would change
	changingPower := int64(0)
	for _, tmVal := range tmValidators {
		if currentVal, exists := currentVals[tmVal.Address]; exists {
			// Validator exists, check if power changes
			if currentVal.VotingPower != tmVal.VotingPower {
				changingPower += abs(currentVal.VotingPower - tmVal.VotingPower)
			}
			// Remove from current vals map to track which validators are removed
			delete(currentVals, tmVal.Address)
		} else {
			// New validator, add its power to the change
			changingPower += tmVal.VotingPower
		}
	}

	// Add power of all validators that would be removed
	for _, val := range currentVals {
		changingPower += val.VotingPower
	}

	// Check if more than 1/3 of voting power changes
	maxChangeAllowed := currentPower / 3
	if changingPower > maxChangeAllowed {
		return fmt.Errorf("too much change in voting power: got %d, max allowed %d",
			changingPower, maxChangeAllowed)
	}
